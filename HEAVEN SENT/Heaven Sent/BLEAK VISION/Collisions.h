	...
	void update(const std::vector<Entity*>& entities) //create vector of entities
	{
		for (const auto& hit : m_hits) // go through the hits vector
		{
			for (auto* entity : entities) // go through the entities vector
			{
				...
				{
					if (entity != hit.ignore && typeid(*entity) != typeid(*hit.entity) && entity != hit.entity) // if entity is not on the ignore list, and entity is not the same type (projectile), and entity is not itself (player/enemy melee hits)
					{
...
private:
	std::vector<HitInfo> m_hits; // vector of hits to check
