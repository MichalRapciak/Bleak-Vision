...
Enemy::Enemy(int wave) : m_enemySprite(m_enemyTexture), m_weaponSprite(m_weaponTxt), m_enemyHorns(m_enemyHornsTxt)
{
	...
	if (!m_enemyHornsTxt.loadFromFile("Assets/Enemies/horns.png"))
	{
		std::cout << "couldnt load horns txt\n";
	}
	m_enemyHorns.setTexture(m_enemyHornsTxt);
	m_enemyHorns.setTextureRect(sf::IntRect({ 0,0 }, { 200,200 }));
	m_enemyHorns.setOrigin({ 100,100 });
	m_enemyHorns.setScale({ 0.3f,0.3f });
	m_enemyHorns.setPosition(m_enemyPosition);
...
void Enemy::update(float dt, Level& level, Player& player)
{
	if (!isDead)
	{
		...
		m_enemySprite.setPosition(m_enemyPosition);
		m_enemyHorns.setPosition(m_enemyPosition);
		m_enemyHorns.setRotation(m_enemySprite.getRotation());
		if (m_weapon) // if a weapon is created, keep it glued to the player
		{
			...
			m_weaponSprite.setScale(m_enemySprite.getScale());
...
/// <summary>
/// This is in charge of moving the entity accordingly to velocity and wall collisions
/// </summary>
...
/// <summary>
///  moves along axis depending on current velocity and walls surrounding
/// </summary>
/// <param name="dx"></param>
/// <param name="dy"></param>
/// <param name="level"></param>
...
/// <summary>
/// picks a random weapon for the enemy
/// </summary>
