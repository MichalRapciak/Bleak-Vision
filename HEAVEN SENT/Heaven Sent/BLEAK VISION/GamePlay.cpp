...
GamePlay::GamePlay() : m_pauseText(m_font), m_playerHealth(m_font), m_gameOverText(m_font), m_gameWonText(m_font)
{
	setupGame();
	// these below set up the player camera
...
void GamePlay::initialise(sf::Font& t_font)
{
	...
	m_gameWonText.setString("You have survived 10 waves of demons.\n Your fate is:\n");
	...
	m_playerHealth.setPosition({ 50,50 });
...
void GamePlay::update(sf::Time& t_deltaTime, sf::RenderWindow& t_window)
{
	...
	if (m_level1.isGameWon())
	{
		m_gameWon = true;
	}
	if (!m_pause && !m_gameOver && !m_level1.isGameWon())
	{
		playerRep = m_player->getReputation();
...
void GamePlay::render(sf::RenderWindow& t_window)
{
	...
	t_window.draw(m_player->getHalo());
	t_window.draw(m_player->getHorns());
	t_window.draw(m_player->getWeaponSprite());
	for (auto& enemy : m_enemies)
	{
		t_window.draw(enemy->getSprite());
		t_window.draw(enemy->getHorns());
...
{
	playerRep = m_player.get()->getReputation();
	if (playerRep <= -2)
	{
		currentAlignment = "Demon";
		m_gameWonText.setString("You have survived 10 waves of demons.\nYour fate is:\nStaying in hell for eternity\nPress Space to return to Main Menu");
	}
	if (playerRep > -2 && playerRep < 2)
	{
		currentAlignment = "Balanced";
		m_gameWonText.setString("You have survived 10 waves of demons.\nYour fate is:\nEscaping Hell\nPress Space to return to Main Menu");
	}
	if (playerRep >= 2)
	{
		currentAlignment = "Angel";
		m_gameWonText.setString("You have survived 10 waves of demons.\nYour fate is:\nTurning Hell into heaven\nPress Space to return to Main Menu");
	}

	m_playerHealth.setString("Health: " + std::to_string(static_cast<int>(m_player.get()->getHealth())) + "/" +  std::to_string(static_cast<int>(m_player.get()->getMaxHealth())) 
		+ "\n" + "Wave: " + std::to_string(m_level1.getCurrentWave() + 1) 
		+ "\n" + "Enemies Left: " + std::to_string(m_level1.getEnemiesLeft())
		+ "\n" + "Current Weapon: " + m_player.get()->getCurrentWeapon()
		+ "\n" + "Souls: " + std::to_string(m_player.get()->getSouls())
