...
void SkillTree::skillTreeUpdate(sf::Time& t_deltaTime, sf::RenderWindow& t_window)
{
	...
	bool currentState = sf::Mouse::isButtonPressed(sf::Mouse::Button::Left); // current mouse state

	for (int i = 0; i < m_skillTreeButtonCount; i++) // go through button vector
	{
		m_skillTreeText[i].setFillColor(sf::Color::White); // turn current button normal
		m_buttonSprite[i].setColor(sf::Color::White);
		if (isMouseOverButton(i, mouseLocation, weaponType::melee) == true) // check if mouse is over current button
		{
			m_buttonSprite[i].setColor(sf::Color{ 100,100,100,255 }); // darken the button
			m_skillTreeText[i].setFillColor(sf::Color{ 75,75,75,255 });
			if (currentState && !m_previousMouseState) // if mouse button state changed (its been pressed), click button
			{
	...
	m_previousMouseState = currentState; // reset mouse button state
}
		...
		for (int j = 0; j < m_weaponUpgradeButtonCount; j++)
		{
			if (isMouseOverButton(i, mouseLocation, static_cast<weaponType>(j)) == true)
			{
				if (static_cast<weaponType>(j) == weaponType::melee)
				{
					...
					sf::String levelandcost = "Level: " + std::to_string(m_player.getWeaponUpgradeLevel(static_cast<WeaponUpgradeType>(i), static_cast<weaponType>(j))) + " Cost: " + std::to_string(m_player.getWeaponUpgradeCost(static_cast<WeaponUpgradeType>(i), static_cast<weaponType>(j))); // display level and cost
